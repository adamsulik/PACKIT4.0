"""
Moduł zawierający implementację algorytmu załadunku wzdłuż osi X oraz Z.
"""
...
class XZAxisLoading(LoadingAlgorithm):
    """
    Algorytm załadunku wzdłuż osi X oraz Z.
    
    Algorytm załadunku optymalizujący wykorzystanie przestrzeni naczepy
    poprzez układanie palet warstwami wzdłuż osi X (długość) i Z (wysokość).
    """
    
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        """
        Inicjalizuje algorytm załadunku wzdłuż osi X oraz Z.
        
        Args:
            config: Słownik konfiguracyjny algorytmu (opcjonalny)
        """
        # Domyślna konfiguracja
        default_config = ALGORITHM_DEFAULTS.get("XZ_Axis_Loading", {})
        
        # Połączenie domyślnej konfiguracji z konfiguracją dostarczoną przez użytkownika
        merged_config = {**default_config, **(config or {})}
        
        super().__init__("XZ Axis Loading", merged_config)
    
    def load_pallets(self, pallets: List[Pallet]) -> List[Pallet]:
        """
        Przeprowadza załadunek palet do naczepy wzdłuż osi X oraz Z.
        ...
        """
        logger.info(f"Rozpoczynam załadunek {len(pallets)} palet metodą XZ_Axis_Loading")
    ...
    def _find_best_position(self, pallet: Pallet, start_position: str) -> Optional[Tuple[int, int, int]]:
        """
        Znajduje najlepszą pozycję dla palety według strategii załadunku wzdłuż osi X oraz Z.
